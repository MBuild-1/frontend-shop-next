...
const ProductBrandWithSlug = props => {
  const { brand, categories, brands, productEntries } = props;
  ...
  const handleLoadMore = async () => {
    ...
    const encryptType = JSON.parse(
      CryptoJSAesEncrypt(
        `/entry?brand=${brand.slug}&pageNumber=20&page=${start}`,
      ),
    );
  ...
  return (
    <>
      <Head>
        <title>{brand.name} Termurah & Kualitas Terbaik | Masterbagasi</title>
      </Head>
      <Navbar />
      <div className="container lg:px-6 lg:mt-20 mt-[8.5rem]">
        <div className="relative">
          <Banner src={'/' + brand.banner_desktop} alt={brand.name} />
            ...
            {productEntries.data.length !== 0 ? (
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-center">
                  Produk Dari Brand {brand.name} Belum Tersedia.
                </p>
              </div>
            )}
          </div>
...
export default ProductBrandWithSlug;
