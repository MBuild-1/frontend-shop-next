...
const ProductCategoryWithSlug = props => {
  const { category, categories, brands, productEntries } = props;

  const breadcrumbs = [
    { href: '/', label: 'Home', icon: HiHome },
    { href: '/product', label: 'Produk' },
    { href: '/product/category', label: 'Kategori' },
    {
  ...
  const handleLoadMore = async () => {
    ...
    const encryptType = JSON.parse(
      CryptoJSAesEncrypt(
        `/entry?category=${category.slug}&pageNumber=20&page=${start}`,
      ),
    );
  ...
  return (
    <>
      <Head>
        <title>
          {category.name} Termurah & Kualitas Terbaik | Masterbagasi
        </title>
      </Head>
      <Navbar />
      <div className="container lg:px-6 lg:mt-20 mt-[8.5rem]">
        <div className="relative">
          <Banner src={'/' + category.banner_desktop} alt={category.name} />
            ...
            {productEntries.data.length !== 0 ? (
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-center">
                  Produk Dari Kategori {category.name} Belum Tersedia.
                </p>
              </div>
            )}
          </div>
...
export default ProductCategoryWithSlug;
