...
export const getServerSideProps = async () => {
  try {
    ...
    const productEntries = await loadProductEntryWithFilter(
      `category=teh dan kopi asli indonesia&pageNumber=20&page=1`,
    );
...
const CoffeeteaPage = props => {
  ...
  const handleLoadMore = async () => {
    ...
    const encryptType = JSON.parse(
      CryptoJSAesEncrypt(
        `/entry?category=teh dan kopi asli indonesia&pageNumber=20&page=${start}`,
      ),
    );
  ...
  return (
    <>
      <Head>
        <title>Produk Teh & Kopi Asli Indonesia | Masterbagasi</title>
      </Head>
      <Navbar />
            ...
            {productEntries.data.length !== 0 ? (
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-center">
                  Produk Dari Kategori Teh & Kopi Asli Indonesia Belum Tersedia.
                </p>
              </div>
            )}
          </div>
