...
export const getServerSideProps = async () => {
  try {
    ...
    const productEntries = await loadProductEntryWithFilter(
      `type=handycrafts&pageNumber=20&page=1`,
    );
...
const HandicraftPage = props => {
  ...
  const breadcrumbs = [
    ...
    { href: '/product', label: 'Produk' },
    {
      ...
      label: 'Kerajinan Tangan Karya Anak Bangsa',
      isCurrentPage: true,
    },
  ];
  ...
  const handleLoadMore = async () => {
    ...
    const encryptType = JSON.parse(
      CryptoJSAesEncrypt(`/entry?type=viral&pageNumber=20&page${start}`),
    );
  ...
  return (
    <>
      <Head>
        <title>Produk Kerajinan Tangan Karya Anak Bangsa | Masterbagasi</title>
      </Head>
      <Navbar />
            ...
            {productEntries.data.length !== 0 ? (
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-center">
                  Produk Dari Kerajinan Tangan Karya Anak Bangsa Belum Tersedia.
                </p>
              </div>
            )}
          </div>
